(function (fs) {
	...
	// the return type of viewScene is a callback to yield control to a minigame co-routine.
	...
	function logError(str,parent) {
	    console.log (str);
	    var div = document.createElement("DIV");
	    div.innerHTML = "Error loading FunkScene file at <a href=\"" + url + "\">" + url + "</a> :<p><code>" + str + "</code>";
	...
	try {
	    ...
	    logError (buildErrorMessage(e), storyParentDiv);
	}
	...
	try {
	    ...
	    logError (e.message, storyParentDiv);
	}
	...
	if (fs.debugging()) {
		...
		logError (buildErrorMessage(e), debugInfoDiv);
	    }
	}
	...
	if (fs.debugging()) {
	    ...
	    debugLooseEndsDiv.innerHTML = "<font color=\"" + firstNodeColor + "\">Current node: <i>" + firstNodeId + "</i></font><br>" + fs.debug.looseEndHtml;
	...
	if (typeof(label) != 'undefined' && label != node.id)
	    attr.unshift ({ attr: "#PAGE", val: label });
