var LetterWriter = (function(){
	...
	nav.changeShowIDE = function() {
		...
		$("#editor,#editorMapContainer,#editorParseTreeContainer,#editorDivider,#showTooltipsCheckboxContainer").show()
		...
		$("#editor,#editorMapContainer,#editorParseTreeContainer,#editorDivider,#showTooltipsCheckboxContainer").hide()
	...
	nav.Message.prototype.populatePlayerList = function() {
	    ...
	    $("#pendingGrammarText").text('Waiting on channel #' + this.channel + ": got " + j + " player(s), need " + n)
	    $("#playerList").empty()
	...
	function activateInvitationListener() {
		...
		// handle message
		var message = new nav.Message(m)
		var desc = message.grammar.title + " (channel #" + message.channel + "): " + message.playerIDs().join(', ')
	...
	if ("timesHidden" in obj) {
	    ...
	    if (n == 1) str = "[hidden once] "
	    else if (n == 2) str = "[hidden twice] "
	    else if (n == 3) str = "[hidden thrice] "
	    else str = "[hidden " + n + "] ";
	}
	return str;
    }
	...
	this.roleControl =
	    addControl ({ property: "role",
			  ...
			  inputDiv: $('<select/>').attr('name','nontermRole'+(++nteDummyCount)).append('<option value="=">Continues</option><option value="+">Passes to next player</option><option value="-">Passes to previous player</option>' + role.map(function(r){return'<option value="'+r+'">Passes to player #'+r+'</option>'}).join('')),
			  hasWarning: false,
...
//	setLocation()
	...
	while (rhsQueue.length) {
	    ...
	    var symNode = currentParentNode.newChild (sym, span, sourceRule);  // creates LetterWriter.Node, attaches to parse tree; if nonrandom, creates controllers & attaches to DOM
...
//	    letter.reveal();
    ...
    lw.Scope = function(parent) { extend (this, { parent:parent, paramValue:{}, role: undefined }) }
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	this.letter.addMoveListener (id, this.nodeChannel(), moveHandler)
	this.letter.addMoveListener (id + '*', this.allDescendantNodeChannels(), moveHandler)
    }
	...
	if ("client" in this.letter) {
	    ...
	    this.letter.client.publish(this.nodeChannel(), msg)
	}
    }
