var LetterWriter = (function(){
	...
	nav.Message.prototype.populatePlayerList = function() {
	    ...
	    $("#pendingGrammarText").text("Players: got " + j + ", need " + n)
	    $("#playerList").empty()
...
//	    letter.reveal();
	...
	if (hint == "")
	    hint = rhs.map(function(sym){return sym instanceof LetterWriter.Term ? sym.text : ""}).join(" ")
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    if (parentNode instanceof LetterWriter.Node) {
		var symbolRole = symbol.nonterminal.role, parentRole = parentNode.symbol.nonterminal.role, totalRoles = letter.grammar.roles
		switch (symbolRole) {
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	var node = this
	while (true) {
	    suffices.unshift (node.symbol.nonterminal.id)
