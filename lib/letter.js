var LetterWriter = (function(){
	...
	addProperty (this, "Pause", "pause", {type:"checkbox",parentDiv:propColumn[c++]}, "If the Pause box is checked, then all text that appears after " + nid + " in any given expansion will be hidden until " + nid + " has been fully expanded.");
	addProperty (this, "Commit", "commit", {type:"checkbox",parentDiv:propColumn[c++]}, "If the Commit box is checked, then after " + nid + " is expanded, the undo history will be cleared: the player will not be able to go back.");
	...
	nontermDiv
	    .append(rulesDiv)
	    .append($('<span class="addRule"></span>').append($('<i></i>').append(addRuleLink)))
	...
	this.hintTitleContainerDiv = $('.HintPropertyEditor')
	this.hintTitleDiv = $('.HintPropertyEditor>div>.propName>i')

/*
	var titleDiv = $('<div class="rule"/>')
	    .append($('<div/>').addClass("hintWarning"))
	    .append(this.hintTitleContainerDiv
		    .append(this.hintTitleDiv))
	    .append($('<div/>').addClass("rhsWarning"))
...
*/
	...
	if (grammarEditor.tooltips)
	    deleteLinkDiv.attr("title","Click to delete this row.").tooltip()
	...
	function validateHint(random) {
		...
		var missing = weightFunc.getMissingParams(grammar)
		if (missing.length)
		    showWarningIcon (hintWarningDiv, errorPrefix + "Missing parameters: " + missing.join(", "))
		else
		...
		if (typeof(newRhs) != 'undefined') {
			...
			for (var i = 0; i < rhs.length; ++i) {
				...
				if (newNonterm.id in grammar.nonterm)
				    newSym.nonterminal = grammar.nonterm[newNonterm.id]
				else {
				    callback (newSym)
		    ...
		    scanRhsForNewSymbols(newRhs,function(sym){
			if (!(sym.nonterminal.id in grammar.nonterm)) {
			    grammarEditor.addNonterm (sym, nonterm)
	...
	if (templateDiv.length)
	    this.notifyChange = chain (this.notifyChange,
				       function() { templateDiv[0].value = grammar.toCanonicalString() })

	function showDebugMap() { grammarEditor.debugMap = new lw.DebugMapView (grammar, mapDiv[0]) }
	if (mapDiv.length) {
	    this.notifyChange = chain (this.notifyChange, showDebugMap)
	    showDebugMap()
	}
...
//	    handle: "h3",
	...
	paramDiv.change(function(e,ui){
	    ...
	    // update
	    if (newId != oldId)
		if (typeof (grammar.findParam(newId)) != "undefined") {
    ...
    lw.GrammarEditor.prototype.notifyChange = function() { }
    ...
    lw.Rule.prototype.weight = function(paramValue) {
	var weight = 1;
	if (letter.ruleActive (this)) {
	    var weightFunc = LetterWriter.ParamFunc.newFromString (this.hint);
	    if (typeof weightFunc != 'undefined' && weightFunc.gotAllParams(letter.grammar)) {
		weight = weightFunc.evaluate(paramValue);
	...
	extend (parentNode.expansionParamValue = {}, parentNode.letter.paramValue);  // record Param settings, for pure interest from an analytics pov
	...
	while (rhsQueue.length) {
		...
		symNode.leaf = false;
		var rule = sym.nonterminal.randomRule (symNode.getParamValue());
	...
	var weightFunc;
	try {
	    weightFunc = LetterWriter.parser.parse (str, "sum_weight_expr");
	} catch (e) {
    ...
    lw.ParamFunc.prototype.evaluate = function(paramValue) {
	switch (this.op) {
	case "*": return this.l.evaluate(paramValue) * this.r.evaluate(paramValue);
	case "/": return this.l.evaluate(paramValue) / this.r.evaluate(paramValue);
	case "+": return this.l.evaluate(paramValue) + this.r.evaluate(paramValue);
	case "-": return this.l.evaluate(paramValue) - this.r.evaluate(paramValue);
	case ".": return String(this.l.evaluate(paramValue)) + String(this.r.evaluate(paramValue));
	case "!": return 1 - this.l.evaluate(paramValue);
	case "$": return paramValue[this.param];
    ...
    lw.ParamFunc.prototype.getMissingParams = function(grammar) {
	var gramParam = {}
	for (var i = 0; i < grammar.params.length; ++i)
	    gramParam[grammar.params[i].id] = true
	var params = this.getParams(), missing = [];
	for (var i = 0; i < params.length; ++i)
	    if (!(params[i] in gramParam))
		missing.push (params[i]);
	return missing;
    }

    lw.ParamFunc.prototype.gotAllParams = function(grammar) {
	return this.getMissingParams(grammar).length == 0;
    }
    ...
    lw.Term.prototype.updateParamValue = function(paramValue,local){}
    ...
    lw.Nonterm.prototype.randomRule = function(paramValue) {
	var total = 0;
	var weight = [];
	for (var i = 0; i < this.rules.length; ++i) {
	    var w = this.rules[i].weight(paramValue);
	    weight.push(w);
	    total += w;
	}
    ...
    lw.NontermReference.prototype.updateParamValue = function(paramValue,local){}
    ...
    lw.ParamReference.prototype.updateParamValue = function(paramValue,local){}
    ...
    lw.ParamAssignment.prototype.updateParamValue = function(paramValue,local) {
	if (local ? this.local : !this.local)
	    paramValue[this.id] = this.value.evaluate (paramValue)
    }
	...
	if (parseDiv.length) {
	    this.notifyChange = chain (this.notifyChange, showDebugTree)
	    showDebugTree()
	}
    }
    
    lw.Letter.prototype.notifyChange = function() { }
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    this.preambleSpan.innerHTML = symbol.preamble();
	    this.placeholderSpan.innerHTML = symbol.placeholder();
	    if (!symbol.random())
	    ...
	    var paramValue = this.getParamValue()
	    if (symbol.id in paramValue)
		this.span.innerHTML = paramValue[symbol.id]
	    else
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
    ...
    lw.Node.prototype.getParamValue = function() {
	var paramValue = {}
	// precedence: sliders > locals > globals
	this.letter.root.iteratePre (function(){
	    this.symbol.updateParamValue (paramValue, false)  // globals
	}, this);
	this.iterateAncestorsPre (function(child){
	    this.iterateChild (function(){
		this.symbol.updateParamValue (paramValue, true)  // locals
	    }, child)
	});
	extend (paramValue, this.letter.paramValue)  // sliders
	return paramValue;
    }
	...
	// TODO: terminals, preambles, placeholders should be squares; nonterminals circles
	// TODO: colors should indicate dead nodes (i.e. nonterminals that have no active rules), pending nodes, hidden nodes
	...
	for (var n = 0; n < this.grammar.nonterms.length; ++n) {
	    ...
	    var id = this.grammar.nonterms[n].id
	...
	for (var lhs in this.grammar.nonterm)
		...
		for (var k = 0; k < this.grammar.nonterm[lhs].rules[j].rhs.length; ++k) {
		    ...
		    if (sym instanceof lw.NontermReference) {
			this.sigInst.addEdge (lhs + "." + sym.nonterminal.id, "@"+lhs, "@"+sym.nonterminal.id)
		    }
		}
	...
	var addNode = function(node,parent,x) {
	    addDebugTreeNode(makeNontermId(node),makeNontermId(parent),node.symbol.nonterminal.asText(),x) }

	var addPreamble = function(parentNode,x) {
	    addDebugTreeNode(makePreambleId(parentNode),makeNontermId(parentNode),makeTextLabel(makePreambleId(parentNode),parentNode.symbol.nonterminal.preamble),x) }

	var addTerm = function(termNode,x) {
	    addDebugTreeNode(makeTermId(termNode),makeNontermId(termNode.parent),makeTermLabel(termNode),x) }

	var addCode = function(node,x) {
	    addDebugTreeNode(makeCodeId(node),makeNontermId(node.parent),makeCodeLabel(node),x) }

	var addDebugTreeNode = function(id,parentId,label,x) {
