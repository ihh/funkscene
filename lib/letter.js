var LetterWriter = (function(){
    ...
    // editor methods
    lw.Grammar.prototype.newEditor = function(parentDiv) {
	return new lw.GrammarEditor (this, parentDiv);
    }

    // NontermEditor
    lw.NontermEditor = function(grammar,nonterm,parentDiv) {
	if (typeof nonterm == 'undefined')
	    nonterm = grammar.start;
	if (typeof parentDiv == 'undefined')
	    parentDiv = document.getElementById ("editor");
	extend (this, { grammar: grammar,
			nonterm: nonterm,
			parentDiv: parentDiv });
	var nontermDiv = this.nontermDiv = document.createElement("DIV");
	var propsDiv = this.propsDiv = document.createElement("DIV");
	var rulesDiv = this.rulesDiv = document.createElement("DIV");
	nontermDiv.setAttribute ("class", "nonterminal");
	propsDiv.setAttribute ("class", "properties");
	rulesDiv.setAttribute ("class", "expansions");
	nontermDiv.appendChild (propsDiv);
	nontermDiv.appendChild (rulesDiv);
	parentDiv.appendChild (nontermDiv);

	this.propDiv = {};
	function addProperty(editor,name,value) {
	    var propDiv = editor.propDiv[name] = {};
	    var propNameValueDiv = propDiv.parent = document.createElement("DIV");
	    var propNameDiv = propDiv.name = document.createElement("DIV");
	    var propValueDiv = propDiv.value = document.createElement("DIV");
	    propNameValueDiv.setAttribute ("class", "property");
	    propNameDiv.setAttribute ("class", "propname");
	    propValueDiv.setAttribute ("class", "propvalue");
	    propNameDiv.innerHTML = name;
	    propValueDiv.innerHTML = value;
	    propNameValueDiv.appendChild (propNameDiv);
	    propNameValueDiv.appendChild (propValueDiv);
	    propsDiv.appendChild (propNameValueDiv);
	}
    ...
    // GrammarEditor
    lw.GrammarEditor = function(grammar,parentDiv) {
	this.nonterm = [];
	for (var i = 0; i < grammar.nonterms.length; ++i) {
	    var nonterm = grammar.nonterms[i];
	    this.nonterm.push (new lw.NontermEditor (grammar, nonterm, parentDiv));
	}
    }
	...
	var rhsNontermNodes = [];
	var lastPauseNode;
	var rhsQueue = this.rhs.slice(0);
	while (rhsQueue.length) {
	    var sym = rhsQueue.shift();
	    if (sym instanceof lw.NontermReference && sym.nonterminal.random) {
		var expansion = sym.nonterminal.randomExpansion (letter);
		rhsQueue.splice.apply (rhsQueue, [0,0].concat(expansion));
		continue;
	    }
    ...
    // Nonterm method to select a random rule and return its RHS
    lw.Nonterm.prototype.randomExpansion = function(letter) {
	var total = 0;
	var weight = [];
	for (var i = 0; i < this.rules.length; ++i) {
	...
	this.paramsDiv = $("#slidersParent")[0];
	this.showParams();
    ...
    lw.Letter.prototype.hideParams = function() { hideElement(this.paramsDiv) }
    lw.Letter.prototype.showParams = function() { showElement(this.paramsDiv) }
