...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	for (var n = 0; n < this.grammar.nonterms.length; ++n) {
	    ...
	    this.sigInst.addNode("@"+id,{ label: "@"+id,
					  color: empty ? "#ff0000" : (orphan ? "#dd0000" : (nonterm.random ? "#ff88ff" : "#00ff00")),
					  size: empty ? 2 : 1,
					  x: -Math.cos(angle)*R,
					  y: -Math.sin(angle)*R })
	}
	...
	var addNode = function(node,parent,x) {
	    addDebugTreeNode(makeNontermId(node),makeNontermId(parent),{label:node.symbol.nonterminal.asText(),x:x,color:node.leaf?"#00ff00":(node.symbol.random()?"#ff88ff":"#ff8888"),size:node.leaf?2:1}) }

	var addPreamble = function(parentNode,x) {
	    addDebugTreeNode(makePreambleId(parentNode),makeNontermId(parentNode),{label:makeTextLabel(makePreambleId(parentNode),parentNode.symbol.nonterminal.preamble),x:x,color:"#ffcc44"}) }

	var addTerm = function(termNode,x) {
	    addDebugTreeNode(makeTermId(termNode),makeNontermId(termNode.parent),{label:makeTermLabel(termNode),x:x,color:"#ffcc44"}) }

	var addCode = function(node,x) {
	    addDebugTreeNode(makeCodeId(node),makeNontermId(node.parent),{label:makeCodeLabel(node),x:x,color:"#0000ff"}) }
