var LetterWriter = (function(){
	...
	incomingMenu.dropit({beforeShow:function(){
		...
		for (var i = 0; i < incoming.length; ++i) {
		    ...
		    incomingOpts.append($("<li/>")
					.append($("<a/>").attr("href","#").text(incoming[i].asText())
						.click(function(e){
						    e.preventDefault();
						    nontermEditor.focus()})))
		}
	...
	outgoingMenu.dropit({beforeShow:function(){
		...
		for (var i = 0; i < outgoing.length; ++i) {
		    ...
		    outgoingOpts.append($("<li/>")
					.append($("<a/>").attr("href","#").text(outgoing[i].asText())
						.click(function(e){
						    e.preventDefault();
						    nontermEditor.focus()})))
		}
	...
	addNontermInner(nonterm,sibling)
    }
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	for (var lhs in this.grammar.nonterm)
		...
		for (var k = 0; k < this.grammar.nonterm[lhs].rules[j].rhs.length; ++k) {
			...
			var edgeId = lhs + "." + rhs
			if (!gotEdge[edgeId]) {
			    this.sigInst.addEdge (edgeId, lhs, rhs, {color:isBare[rhs] ? "#ff0000" : (this.grammar.nonterm[lhs].random ? "#eeaaaa" : "#aa8888")})
			    gotEdge[edgeId] = true
			}
		    }
		}
