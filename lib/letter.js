var LetterWriter = (function(){
	...
	for (var ruleId in this.ruleDivs)
	    this.ruleDivs[ruleId].rule.remove()
	this.ruleDivs = []
	...
	for (var i = 0; i < this.nonterm.rules.length; ++i) {
	    var rule = this.nonterm.rules[i];
	    this.ruleDivs[rule.id].rhsInput.val(rule.rhsAsText())
	}
    }
...
//	    handle: "h3",
	...
	for (var otherId in this.nontermEditor)
	    this.nontermEditor[otherId].refreshExpansions()
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	// TODO: fix/test/debug this
	// TODO: show all terminals that are not whitespace, including (but not limited to) preamble
	// NB for leaf nodes, the preamble (if there is one) is the only terminal
	// this needs work

	// children-together layout
	// first, count nonterminals...
	// We should also count ALL nodes here, i.e. including Term nodes
	var N = letter.nontermUsage()
	// We need a method to do this
	// var T = letter.termUsage()...
	// then use (2*N+T) in place of (2*N) below
	// for the total number of nodes in the parse tree,
	// including all the preambles (of which there are N)
	// (to be really particular, count the <=N nonempty preambles...)
	var n = 0
	var addNode = function(node,parentId,x) {
	    addDebugTreeNode(node.count,parentId,"@"+node.symbol.nonterminal.id,x) }

	var addPreamble = function(node,x) {
	    addDebugTreeNode(node.count+".preamble",node.count,node.symbol.nonterminal.preamble,x) }
...
//		console.log("Adding edge from "+id+" to "+parentId)
	...
	addNode (letter.root, undefined, .5, 0)

	var iterate = function() {
	    var node = this
//	    console.log(this)
	    var K = this.nontermChildren().length
	    var c = 0
	    addPreamble (this, c++/(K+1))
	    // need to iterate over ALL children here, not just nonterms
	    node.iterateNontermChild(function(){ addNode(this,node.count,c++/(K+1)) })
	    // here, iterating over just nonterms is what we want
	    node.iterateNontermChild(iterate) }

	iterate.apply (letter.root)
