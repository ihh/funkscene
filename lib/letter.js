...
//	addProperty (this, "Phrase", "@" + nonterm.id);
	...
	addProperty (this, "Prompt", "prompt", "textarea", "The prompt is the question asked to the player whenever " + nid + " occurs in the narrative, prompting them to choose an expansion for " + nid + ".");
	...
	var hintInputDiv = $('<textarea rows="1" columns="30"/>').attr({name: "hint" + rule.id}).val(rule.hint);
	...
	if (this.nonterm.random)
		...
		var weightFunc = LetterWriter.ParamFunc.newFromString (rule.hint, "sum_expr");
		if (typeof(weightFunc) != 'undefined' && weightFunc.hasParam(oldParam)) {
		    weightFunc.replace(oldParam,newParam)
		    rule.hint = weightFunc.asText()
	...
	for (var i = 0; i < grammar.params.length; ++i) {
		...
		attr("name","paramName#"+(i+1)).attr("value",param.id)
		...
		.attr("name","paramMin#"+(i+1)).attr("value",param.min)
		.change(function(e,ui){param.min = e.target.value})
	    var paramMaxDiv = $('<input type="text"/>')
		.attr("name","paramMax#"+(i+1)).attr("value",param.max)
	    ...
	    var paramLabelSpan = $('<span style="text-align:right"/>').append(' <i>Initial value:</i> ')
	    var paramContainerDiv = $('<div/>').addClass("editorSliderContainer")
		.append("<i>Quality:</i> ").append(paramDiv)
		.append(" <i>Never:</i> ").append(paramMinDiv)
		.append(" <i>Always:</i> ").append(paramMaxDiv)
		.append($('<div/>').addClass("editorSliderAndLabel").append(paramLabelSpan).append(paramSliderDiv));
