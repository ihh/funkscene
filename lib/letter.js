var LetterWriter = (function(){
    ...
    lw.defaultNever = "Never";
    lw.defaultAlways = "Always";
	...
	// TODO: uncomment next line, allow user to rename phrases seamlessly just by typing in a new name
//	addProperty (this, "Phrase", "@" + nonterm.id);
	...
	var deleteLinkDiv = $('<i><a href="#">Delete option</a></i>')
	    .addClass("delete")
	    .attr("title","Click to delete this row forever, no last chances. Well, one annoying 'are you sure' (if you've typed anything here).").tooltip()

	var hintInputDiv = $('<textarea rows="1"/>').attr({name: "hint" + rule.id}).val(rule.hint);
	var rhsInputDiv = $('<textarea rows="1"/>').attr({name: "rhs" + rule.id}).val(rule.rhsAsText());
...
//	    handle: "h3",
	...
	var paramDeleteLink = $('<a href="#">Delete this slider</a>')
	    .attr("title","Click to delete this slider, no last chances. Well, one annoying 'are you sure' (if any computer hints depend on it).").tooltip()
	...
	var paramContainerDiv = $('<div/>').addClass("editorSliderContainer")
	    ...
	    .append($('<div/>').addClass("editorSliderAndLabel").append(paramSliderDiv).append(paramLabelSpan))
