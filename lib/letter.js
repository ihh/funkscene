var LetterWriter = (function(){
	...
	nav.restartLetterFromGrammar = function() {
	    nav.removeLetter()
	...
	nav.loadInitialGrammar = function() {
	...
	nav.Message.gramhash8 = function() { return this.grammar.hash.substring(0,8) }
	nav.Message.hashchan = function(prefix,suffix) { return '/' + prefix + '/' + this.gramhash8() + '/' + this.channel + (typeof(suffix) === 'undefined' ? "" : ('/' + suffix)) }
	nav.Message.playerIDs = function() { return this.player.map(function(p){return p.id}) }
	nav.Message.playerFingerprints = function() { return this.player.map(function(p){return p.fingerprint}) }
	nav.Message.inviteChannel = function() { return this.hashchan('invite',this.playerFingerprints()) }
	nav.Message.applyChannel = function() { return this.hashchan('apply',this.playerFingerprints()) }
	nav.Message.playerQuitChannel = function() { return this.hashchan('quit',['~'+this.playerRole(),playerFingerprint()]) }
	nav.Message.playerRole = function() { return $.inArray(playerID(),this.playerIDs()) }
	nav.Message.grammarChannel = function() { return '/grammar/' + this.gramhash8() }
	...
	function activateApplicationListener(msg) {
	    ...
	    listener['application'] = client.subscribe (msg.applyChannel(),
							function(msg) {
							    // TODO: write ApplicationListener
							})
	}
	function cancelApplicationListener() { cancelListener('application') }

	function activateRequestListener(msg) {
	    var gc = msg.grammarChannel()
	...
	function activateQuitListener(msg) {
	    var qc = msg.playerQuitChannel(), pr = msg.playerRole()
	    listener['quit'] = client.subscribe (msg.hashchan('quit'),
						 function(msg) {
						     deactivateAll()
	...
	nav.startGame = function(e) {
		...
		var msg = new nav.Message ({ channel: channel,
					     grammar: { name: grammar.name,
							roles: grammar.roles,
							hash: grammar.md5hash() },
					     player: [ { id: playerID(), fingerprint: playerFingerprint() } ] })

		client.publish(msg.inviteChannel(), msg).then(function() {
		    console.log('Message received by server!');
		    activateInvitationPublisher(msg)
		    activateApplicationListener(msg)
