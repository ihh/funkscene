var LetterWriter = (function(){
    ...
    var nontermAnchorPrefix = "nonterm_"
	...
	addProperty (this, "Phrase @", "id", {type:"text",keepFocus:false,parentDiv:propColumn[c++]}, "The word used to identify this phrase.", grammarEditor.nontermRenamingFilter(nonterm))
...
//	addProperty (this, "Prompt", "prompt", {type:"textarea"}, "The prompt is the question asked to the player whenever " + nid + " occurs in the narrative, prompting them to choose from the various hints, each of which leads to an expansion for " + nid + ".");
	...
	document.location = "#" + nontermAnchorPrefix + this.nonterm.id
    }
	...
	var orphan = this.nonterm.isOrphan(this.grammar), empty = this.nonterm.isBare()
	if (orphan && empty) this.anchor.append(" (orphan, bare)")
	else if (orphan) this.anchor.append(" (orphan)")
	else if (empty) this.anchor.append(" (bare)")
    }
	...
	this.sortLink = $("<a/>").attr("href","#").addClass("editorSortLink")
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	for (var n = 0; n < this.grammar.nonterms.length; ++n) {
	    ...
	    var id = nonterm.id
	    var empty = isBare[id] = nonterm.isBare()
	...
	for (var lhs in this.grammar.nonterm)
		...
		for (var k = 0; k < this.grammar.nonterm[lhs].rules[j].rhs.length; ++k) {
			...
			this.sigInst.addEdge (lhs + "." + sym.nonterminal.id, lhs, rhs, {color:isBare[rhs] ? "#ff0000" : (this.grammar.nonterm[lhs].random ? "#eeaaaa" : "#aa8888")})
		    }
		}
	...
	this.sigInst
		...
		var neighbors = {};
		debugMapView.sigInst.iterEdges(function(e){
