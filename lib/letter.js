var LetterWriter = (function(){
	...
	function makeRhsExpr(placeholder,prompt,rules) {
		...
		if (rule.hint.length || "maxUsage" in rule) {
		    str += rule.hint;
		    if ("maxUsage" in rule) str += "[most " + rule.maxUsage + "]";
		    str += "=>";
		}
		str += rule.rhs.map(function(sym){
		    if (sym instanceof LetterWriter.NontermReference) {
			if (sym.nonterminal.anonymous) {
			    if (normalize)
				return "@" + anonId[sym.nonterminal.id] + " ";
			    return makeRhsExpr (sym.placeholder, sym.prompt, sym.nonterminal.rules);
			}
			return "@" + sym.nonterminal.id + " ";
		    } else
			return sym.text;
		}).join("");
	    }
	...
	for (var i = 0; i < this.nonterms.length; ++i) {
		...
		str += "=>" + makeRhsExpr (nonterm.placeholder, nonterm.prompt, nonterm.rules) + "\n";
	    }
	}
