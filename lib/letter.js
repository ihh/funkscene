var LetterWriter = (function(){
	...
	nav.Message.prototype.allPlayChannels = function() { return this.playChannelPrefix() + '/**' }
	...
	function activateApplicationListener(msg) {
		...
		if (message.gotAllPlayers()) {
		    ...
		    nav.restartLetterFromGrammar ({ letterProperties: { gameMessage: message,
									playerRole: message.playerRole() - 1,
									client: client,
									addMoveListener: addMoveListener,
									cancelMoveListener: cancelMoveListener} })
		    // FIXME: instead of calling hideMultiplayerMenu() (which deactivates all listeners), this should change the 'menu' link to 'quit', hide the 'restart' button, hide the multiplayer menu & show the game div
	...
	function activateGrammarListener(msg) {
		...
		nav.loadInitialGrammar (m.grammar.text,
					{ letterProperties: { gameMessage: message,
							      playerRole: message.playerRole() - 1,
							      client: client,
							      addMoveListener: addMoveListener,
							      cancelMoveListener: cancelMoveListener } })
	...
	function activateGameListener(msg) {
	    cancelGameListener()
	    listener['game'] = client.subscribe (msg.allPlayChannels(), function(m) {
	...
	function activateConflictListener(msg,expectedMsg) {
	    cancelConflictListener()
	    listener['conflict'] = client.subscribe (msg.allPlayChannels(),
						     function(newMsg) {
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	if ("client" in this.letter) {
	    ...
	    this.letter.client.publish(this.nodePublishChannel(), msg)
	}
    }
