var LetterWriter = (function(){
	...
	var parentDiv = $("#"+parentId)
		...
		grammar.nonterms = parentDiv.sortable ("toArray", {attribute: nontermIdAttr});
	    }});
	extend (this, { grammar: grammar,
			nonterm: nonterm,
			parentId: parentId,
			parentDiv: parentDiv });
	var listItem = this.listItem = $('<li/>').attr(nontermIdAttr,nonterm.id);
	...
	listItem.append(nontermDiv);
	parentDiv.append(listItem[0]);
	...
	function addProperty(editor,name,value) {
	    ...
	    propNameDiv[0].innerHTML = name;
	    propValueDiv[0].innerHTML = value;
