var LetterWriter = (function(){
	...
	// multiplayer
	var playerID() = function() { return "Player" }
	var playerFingerprint() = function() { return "player" }
	...
	nav.Message.prototype.hashchan = function(prefix,suffix) { return '/' + prefix + '/' + this.gramhash8() + '/' + this.channel + (typeof(suffix) === 'undefined' ? "" : ('/' + suffix)) }
	...
	nav.Message.prototype.applySubscribeChannel = function() { return this.hashchan('apply',this.playerFingerprints()) }
	nav.Message.prototype.applyPublishChannel = function() { var p = this.playerFingerprints(); p.push(nav.playerFingerprint()); return this.hashchan('apply',p) }
	...
	nav.Message.prototype.playChannel = function(nodePathArray) { var a = this.playerFingerprints().concat(nodePathArray); return this.hashchan('play',a) }
	nav.Message.prototype.allPlayChannels = function() { return this.playChannel(['**']) }
	nav.Message.prototype.playersNeeded = function() { return this.grammar.role.length - this.player.length }
	nav.Message.prototype.gotAllPlayers = function() { return playersNeeded() == 0 }
	nav.Message.prototype.populatePlayerList = function() {
	    $("#pendingGrammarName") = this.grammar.name
	    var n = this.playersNeeded()
	    $("#pendingGrammarText") = "Need " + n + " more player" + (n==1 ? "" : "s")
	    $("#playerList").empty()
	...
	function activateApplicationListener(msg) {
		...
		if (m.gotAllPlayers()) {
		    nav.restartLetterFromGrammar ({ player: m.player })
	...
	function activateQuitListener(msg) {
	    ...
	    listener['quit'] = client.subscribe (msg.quitSubscribeChannel(),
						 function(msg) {
						     deactivateAll()
						     client.publish(qc, { channel: msg.channel,
									  grammar: msg.grammar,
									  quit: { role: pr } }) })
	}
	...
	function activateInvitationListener() {
		...
		if (message.player.length == message.grammar.role.length)
	...
	function activateGrammarListener(msg,playerList) {
	    ...
	    listener['grammar'] = client.subscribe (msg.grammarSubscribeChannel(),
						    function(m) {
							cancelGrammarListener()
							nav.loadInitialGrammar (m.grammar.text, { player: playerList }) })
	}
	function cancelGrammarListener(msg) { cancelListener('grammar') }
	...
	function cancelGameListener(msg) { cancelListener('game') }
	...
	function cancelConflictListener(msg) { cancelListener('conflict') }
