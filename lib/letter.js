var LetterWriter = (function(){
	...
	this.preambleControl =
	    addControl ({property: "preamble",
			 ...
			 change: function(){nontermEditor.validatePlainText(nontermEditor.preambleControl.inputDiv.val(),nontermEditor.preambleControl.warningDiv,"The preamble is not expanded or preprocessed at all, and so")},
	...
	this.placeholderControl =
	    addControl ({property: "placeholder",
			 ...
			 change: function(){nontermEditor.validatePlainText(nontermEditor.placeholderControl.inputDiv.val(),nontermEditor.placeholderControl.warningDiv,"The placeholder is not expanded or preprocessed at all, and so")},
	...
	this.promptControl =
	    addControl ({property: "prompt",
			 inputDiv: $('<textarea rows="1"/>'),
			 change: function(){nontermEditor.validatePlainText(nontermEditor.promptControl.inputDiv.val(),nontermEditor.promptControl.warningDiv,"The prompt is not expanded or preprocessed at all, and so")},
...
//	addProperty (this, "Prompt", "prompt", {type:"textarea"}, "The prompt is the question asked to the player whenever " + nid + " occurs in the narrative, prompting them to choose from the various hints, each of which leads to an expansion for " + nid + ".");
    ...
    lw.NontermEditor.prototype.validatePlainText = function(text,warningDiv,prefix) {
		if (/[\S]/.test(text)) {
	...
	function validateHint(random) {
		...
		nontermEditor.validatePlainText(rule.hint,hintWarningDiv,"For phrases whose expansion is chosen by the player, the hint will not itself be expanded, and so")
	}
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	var addNode = function(node,parent,x) {
	    var attr = {Type:"Phrase",Player:node.symbol.random()?"Computer":"Human",Expanded:node.leaf?"not yet":"yes"}
	    var activeRules = 0
	    if (node.leaf && node.symbol instanceof LetterWriter.NontermReference) {
