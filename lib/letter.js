var LetterWriter = (function(){
	...
	var preambleDiv = this.preambleDiv = $('<textarea rows="6"/>').addClass('main')
	    ...
	    .change(function(e){ nonterm.preamble = e.target.value; notify() });

	var placeholderDiv = this.placeholderDiv = $('<textarea rows="1"/>').addClass('main')
	    ...
	    .change(function(e){ nonterm.placeholder = e.target.value; notify() });
	...
	if (!isStart) nontermDiv.append(deleteNontermLink);
	...
	function addProperty(editor,name,property,controlInfo,tooltip,filter) {
	    ...
	    var notify = editor.grammarEditor.notifyChange;
	    var value = (property in nonterm) ? nonterm[property] : "";
	    var propDiv = editor.propDiv[property] = {};
	    var propNameValueDiv = propDiv.parent = $('<div/>')
		.addClass("property")
		.addClass(property+"Property")
		.attr("title",tooltip).tooltip();
		...
		inputDiv.val(value);
		inputDiv.change(function(e){ nonterm[property] = filter(e.target.value); notify() });
	    } else {
		inputDiv.attr("value","x");
		if (value) inputDiv.attr("checked","checked");
		inputDiv.change(function(e){ nonterm[property] = filter(e.target.checked); notify() });
	    }
	...
	var hintWarningDiv = $('<div/>').addClass("hintWarning");
	var rhsWarningDiv = $('<div/>').addClass("rhsWarning");
	...
	var validateHint = function(random) {
	...
	ruleDiv
	    .append(hintWarningDiv).append(hintDiv)
	    .append(rhsWarningDiv).append(rhsDiv)
	    .append(limitDiv).append(deleteLinkDiv);
	this.rulesDiv.append (ruleDiv);

	this.ruleEditor[rule.id] = ({ rule: ruleDiv,
				    hint: hintDiv,
				    hintInput: hintInputDiv,
				    rhs: rhsDiv,
				    rhsInput: rhsInputDiv,
				    hintWarning: hintWarningDiv,
				    rhsInputWarning: rhsWarningDiv,
