...
//	setLocation()
	...
	while (rhsQueue.length) {
	    ...
	    if (sym instanceof lw.NontermReference && symNode.isLocallyControlledByComputer()) {  // PUBSUB TODO: check node is visible (typeof (lastPauseNode) == "undefined")); if hidden, chain a call to sym.nonterminal.randomRule(symNode.getScope()).expand(symSpan,symSpan.placeholderSpan,symNode) along with the faders (and remember to publish)
		...
		if (rule instanceof lw.Rule) {
		    ...
		    // no need to broadcast computer-expanded rules if they are auto-expanded as part of a click event
		    // on completion, the click handler that called expand() will publish the whole tree
		} else {
...
//	    letter.reveal();
	...
	if (this.hasActiveRules (letter)) {
		...
		for (var i = 0; i < this.rules.length; ++i)
			...
			option.value = i + 1;
			option.text = rule.hint.length ? rule.hint : rule.rhs.map(function(sym){return sym instanceof LetterWriter.Term ? sym.text : ""}).join(" ")
	...
	return this.type + this.id + " " + (this.local ? "@=" : "=") + " " + this.value.asText() + ";"
    }
	...
	if (('scoreParam' in grammar) && typeof(grammar.scoreParam) != 'undefined' && typeof(scoreContainerDiv) != 'undefined') {
	    var scoreParams = []
		...
		var updateScore = function() {
			...
			if (scoreDefined) {
				...
				deltaSpan.innerText = (originalDelta[r] > 0 ? "+" : "-") + Math.abs(originalDelta[r]).toString()
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	var node = this
	while (true) {
	    suffices.unshift ((node.symbol instanceof lw.NontermReference) ? node.symbol.nonterminal.id : ('-node-' + node.count))
