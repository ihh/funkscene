var LetterWriter = (function(){
	...
	this.createAnonId = function() {
	    var newId;
	    while ((newId = lw.anonymousNonterminalPrefix + (++anonCount)) in grammar.nonterm) { }
	    return newId;
	}
    }
	...
	for (var i = 0; i < this.nonterms.length; ++i) {
		...
		str += "@" + id + " ";
		if ("maxUsage" in nonterm) str += "[most " + nonterm.maxUsage + "] ";  // TODO: once, twice, thrice, share code with makeRhsExpr
		str += "=> " + makeRhsExpr ({placeholder: nonterm.placeholder == lw.Nonterm.prototype.placeholder ? "" : nonterm.placeholder,
	...
	this.propDiv = {};
	function addProperty(editor,name,property,type) {
		...
		inputDiv.val(value);
		inputDiv.change(function(e){ nonterm[property] = e.target.val() });
	    } else {
		inputDiv.attr("value","x");
		if (value) inputDiv.attr("checked","checked");
		inputDiv.change(function(e){ nonterm[property] = e.target.checked });
	    }
...
//	addProperty (this, "Phrase", "@" + nonterm.id);
	addProperty (this, "Placeholder", "placeholder", "textarea");
	addProperty (this, "Prompt", "prompt", "text");
	addProperty (this, "Pause", "pause", "checkbox");
	addProperty (this, "Commit", "commit", "checkbox");
	addProperty (this, "Random", "random", "checkbox");
	addProperty (this, "Limit", "maxUsage", "text");

	this.hintTitleDiv = $('<div class="hint title"><i>Hint</i></div>');
	var titleDiv = $('<div class="rule"/>').append(this.hintTitleDiv).append('<div class="rhs title"><i>Expands to</i></div><div class="limit title"><i>Limit</i></div>');
	...
	var rhsAsText = rule.rhs.map(function(sym){return sym.asText()}).join("");
	...
	var rhsInputDiv = $('<textarea rows="1" columns="80"/>').attr({name: "rhs" + rule.id}).val(rhsAsText);
	...
	rhsInputDiv.change(function(e){
		...
		// TODO: update rule.rhs
		// loop through newRhs symbols:
		//   anonymous? call grammar.createAnonId
		//   new nonterminal? insert into list, right after this one
	    } catch (e) {
	...
	// NB for rhs, we will need to parse the input using a sanitized subgrammar with no anonymous nonterminals
...
// uncomment to make entire nonterminal div draggable
	...
	function propAccessor(name) { return function() { return (name in myProps) ? myProps[name] : nonterm[name] } }
