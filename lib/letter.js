var LetterWriter = (function(){
	...
	nav.changeShowIDE = function() {
		...
		$("#scoreContainer").attr("style","left:20%")
	    }
	}
	...
	// multiplayer
	function playerID() { return "Player" }
	function playerFingerprint() { return "player" }
	function playerBlob() { return { id: playerID(), fingerprint: playerFingerprint() } }

	// pub/sub message object
	nav.Message = function(message) { extend(this,message) }
	nav.Message.prototype.gramHash8 = function() { return this.grammar.hash.substring(0,8) }  // TODO: cache this for performance?
	nav.Message.prototype.hashChan = function(prefix) { return '/' + prefix + '/' + this.gramHash8() + '/' + this.channel }
	nav.Message.prototype.hashChanPlayer = function(prefix,playerFingerprints) { return this.hashChan(prefix) + playerFingerprints.map(function(s){return'/'+s}).join('') }
	...
	nav.Message.prototype.grammarPublishChannel = function() { return '/grammar/' + this.gramHash8() }
	...
	nav.Message.prototype.inviteChannel = function() { return this.hashChan('invite',this.playerFingerprints()) }
	nav.Message.prototype.applySubscribeChannel = function() { return this.hashChanPlayer('apply',this.playerFingerprints()) }
	nav.Message.prototype.applyPublishChannel = function() { return this.hashChanPlayer('apply',this.playerFingerprints().concat([nav.playerFingerprint()])) }
	nav.Message.prototype.quitSubscribeChannel = function() { return this.hashChan('quit') }
	nav.Message.prototype.quitPublishChannel = function() { return this.hashChan('quit') + '~' + this.playerRole() + '/' + playerFingerprint() }
	nav.Message.prototype.playerRole = function() { return 1 + $.inArray(playerID(),this.playerIDs()) }
	nav.Message.prototype.playChannel = function(nodePathArray) { var a = this.playerFingerprints().concat(nodePathArray).concat(['**']); return this.hashChan('play',a) }
	nav.Message.prototype.allPlayChannels = function() { return this.playChannel([]) }
	nav.Message.prototype.playersNeeded = function() { return this.grammar.roles - this.player.length }
	nav.Message.prototype.gotAllPlayers = function() { return playersNeeded() == 0 }
	nav.Message.prototype.populatePlayerList = function() {
	    $("#pendingGrammarName").text(this.grammar.title)
	    var n = this.playersNeeded()
	    $("#pendingGrammarText").text("Need " + n + " more player" + (n==1 ? "" : "s"))
	    $("#playerList").empty()
	    for (var p in this.player) { $("#playerList").append($('<li/>').text(p.id)) }
	}

	function clearPlayerList() {
	    $("#pendingGrammarName").text('')
	...
	function addMoveListener(type,channel,callback) {
	    moveListener[type] = client.subscribe(channel,callback)
	}

	var invitationDelay = 1000
	function activateInvitationPublisher(msg) {
	    publisher['invitation'] = setInterval (invitationDelay, function() {
		client.publish(msg.inviteChannel(), msg)
	    })
	}
	...
	function activateApplicationListener(msg) {
		...
		m.populatePlayerList()
		if (m.gotAllPlayers()) {
		    nav.restartLetterFromGrammar ({ gameMessage: m,
						    playerRole: m.playerRole() - 1,
						    client: client,
						    addMoveListener: addMoveListener,
						    cancelMoveListener: cancelMoveListener})
		    nav.hideMultiplayerMenu()
		} else {
		    listener['application'] = client.subscribe (m.applySubscribeChannel(), applicationListener)
		    activateInvitationPublisher(m)
		}
	    }
	...
	function quitMultiplayerFunction(msg) {
	    var qc = msg.quitPublishChannel(), pr = msg.playerRole()
	    var c = msg.channel, g = msg.grammar
	    return function() {
		deactivateAll()
	...
	function activateQuitListener(msg) {
	    var qc = msg.quitPublishChannel(), pr = msg.playerRole()
	    listener['quit'] = client.subscribe (msg.quitSubscribeChannel(),
						 quitMultiplayerFunction(msg))
	}

	function showMultiplayerMenu() { 
	    $("#letter,#slidersContainerParent,#scoreContainerParent,#topLinksContainer").hide()
	...
	function activateInvitationListener() {
		...
		var desc = message.name + " (" + message.playerIDs().join() + " )"
		...
		var link = $('<a/>').attr("href","#").addClass("joinGameLink").text(desc)
		if (message.player.length == message.grammar.roles)
		    link.onclick(function(e){ nav.joinGame(message) })
		elt.empty().append(link)
	    })
	}
	function cancelInvitationListener() { cancelListener('invitation') }

	function activateGrammarListener(msg,playerList) {
	    cancelGrammarListener()
	    listener['grammar']
		= client.subscribe (msg.grammarSubscribeChannel(),
				    function(m) {
					cancelGrammarListener()
	...
	function activateGameListener(msg) {
	    cancelGameListener()
	    listener['game'] = client.subscribe (msg.allPlayChannels(), function(newMsg) {
		cancelConflictListener()
		cancelGameListener()
		newMsg.populatePlayerList()
		if (newMsg.gotAllPlayers()) {
		    activateQuitListener(newMsg)
		    activateGrammarListener(newMsg)
		} else
		    activateGameListener(newMsg)
	    })
	}
	function cancelGameListener() { cancelListener('game') }

	function activateConflictListener(msg,expectedMsg) {
	    cancelConflictListener()
	    listener['conflict'] = function(newMsg) {
	...
	nav.joinGame = function(msg) {
	    ...
	    client.publish (msg.applyPublishChannel(), msg)
	    activateConflictListener(msg,newMsg)
	    activateGameListener(newMsg)
	}

	var quitMultiplayer = function() { }
	nav.quitMultiplayer = function() {
	    quitMultiplayer()
	...
	nav.startGame = function(e) {
	    ...
	    $("#pendingMultiplayer").show()
	    clearPlayerList()
	    $.get("id",{},function(channel) {
		var msg = new nav.Message ({ channel: channel,
					     grammar: { title: grammar.title,
							roles: grammar.roles,
							hash: grammar.md5hash() },
					     player: [ playerBlob() ] })
		quitMultiplayer = chain (quitMultiplayer, quitMultiplayerFunction(msg))
		msg.populatePlayerList()
	...
	this.roleControl =
	    addControl ({ property: "role",
			  parentDiv: propColumn[c++],
			  name: "Turn",
			  inputDiv: isStart ? " #1" : $('<select/>').attr('name','nontermRole'+(++nteDummyCount)).append('<option value="=">Continues</option><option value="+">Passes to next player</option><option value="-">Passes to previous player</option>' + role.map(function(r){return'<option value="'+r+'">Passes to player #'+r+'</option>'}).join('')),
			  hasWarning: false,
			  getInput: function(){return isStart ? false : this.inputDiv.val()},
			  setInput: function(x){if (!isStart) this.inputDiv.val(x)},
...
//	setLocation()
	...
	function validateHint(random) {
		...
		nontermEditor.validatePlainText(function(){return hintWarningDiv},"For choices whose expansion is chosen by the player, the hint will not itself be expanded, and so")(rule.hint)
	}
	...
	while (rhsQueue.length) {
	    ...
	    if (sym instanceof lw.NontermReference && symNode.isLocallyControlledByComputer()) {  // TODO: check node is visible (typeof (lastPauseNode) == "undefined")); if hidden, chain a call to sym.nonterminal.randomRule(symNode.getScope()).expand(symSpan,symSpan.placeholderSpan,symNode) along with the faders
...
//	    letter.reveal();
    ...
    lw.Nonterm.prototype.addRule = function(hint,rhs,maxUsage) {
	var ruleIndex = this.rules.length
	this.rules.push(new lw.Rule(this.newRuleId(),hint,this,rhs,maxUsage))
	...
	if (this.hasActiveRules (letter)) {
	    function broadcast() {
		var msg = new lw.Navigator.Message (letter.gameMessage)
		msg.tree = letter.root.treeJSON()
		letter.client.publish(parentNode.nodeChannel(), msg)
	    }
	    if (this.isHyperlink()) {
    ...
    lw.Nonterm.prototype.attachWithListener = function (parentNode, parentSpan, placeholderSpan) {
	parentNode.addMoveListener();
	parentSpan.appendChild (placeholderSpan);
    }
	...
	var scoreLabelDiv = document.getElementById('scoreLabel')
	var scoreContainerDiv = document.getElementById('scoreContainer')
	if (typeof(grammar.scoreParam) != 'undefined' && typeof(scoreDiv) != 'undefined') {
	    scoreDiv.innerHTML = ""
	    var scoreParamFunc = new lw.ParamFunc ({ op: '$', param: grammar.scoreParam })
	    this.getScore = function() { return scoreParamFunc.evaluate (letter.root.getFinalScope()) }
		...
		var updateScore = function() {
		    ...
		    var result = ""
		    var label = ""
		    var score = letter.getScore()
		    var scoreDefined = typeof(score) != 'undefined'
		    var sgn = 0
		    if (scoreDefined) {
			label = "Score:"
			var oldScore = parseInt (scoreDiv.innerText) || 0
			var newScore = score
			if (score != oldScore) {
			    var delta = score - oldScore
			    if (typeof(originalDelta) == 'undefined')
				originalDelta = delta
			    sgn = sign(delta)
			...
			} else if (typeof(originalDelta) != 'undefined') {
			    var deltaSpan = document.createElement("span")
			    deltaSpan.setAttribute("class",originalDelta > 0 ? "scoreDeltaPositive" : "scoreDeltaNegative")
			    deltaSpan.innerText = (originalDelta > 0 ? "+" : "-") + Math.abs(originalDelta).toString()
			    var deltaSpanPos = -5, deltaSpanOpacity = 1
			    function setPos() { deltaSpan.setAttribute("style","top:"+deltaSpanPos+";opacity:"+deltaSpanOpacity) }
			    setPos()
			    scoreContainerDiv.appendChild(deltaSpan)
			    var maxTicks = 80, ticks = maxTicks
			    var deltaTimer = window.setInterval(function(){
				if (--ticks <= 0) {
		    ...
		    scoreContainerDiv.setAttribute ("class", scoreDefined ? (sgn == 0 ? "scoreContainer" : (sgn > 0 ? "scoreContainerIncreasing" : "scoreContainerDecreasing")) : "scoreContainerHidden")
		    scoreDiv.innerText = result
		    scoreLabelDiv.innerText = label
		}
	...
	// OK, ready to go
	this.root = new LetterWriter.Node (this, new LetterWriter.NontermReference (grammar.start, {}), undefined);
	...
	if (this.grammar.isSinglePlayer())
	...
	if (typeof(scoreContainerDiv) != 'undefined')
	    scoreContainerDiv.setAttribute ("class", "scoreContainerHidden")
	if (typeof(scoreLabelDiv) != 'undefined')
	    scoreLabelDiv.innerText = ""
	if (typeof(scoreDiv) != 'undefined')
	    scoreDiv.innerText = ""
    }
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    if (parentNode instanceof LetterWriter.Node) {
		var symbolRole = symbol.nonterm.role, parentRole = parentNode.symbol.nonterm.role, totalRoles = letter.grammar.roles
		switch (symbolRole) {
	...
	return ("playerRole" in this.letter) ? (this.letter.playerRole == this.role) : true;
    }

    lw.Node.prototype.isLocallyControlledByHuman = function() {
	return this.isLocallyControlled() && !this.symbol.random();
    }
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
    ...
    lw.Node.prototype.getScopeInner = function() {
	var node = this
	var finalScope
	...
	updateScope (node.letter.root, this, finalScope = new lw.Scope())
	return finalScope
    }

    lw.Node.prototype.getScope = function() {
	var sliderScope = new lw.Scope (this.getScopeInner())
	...
	var lastDescendant = this.getLastDescendant()
	var finalScope = new lw.Scope (lastDescendant.getScope())
	if (lastDescendant.symbol instanceof lw.ParamAssignment)
	    lastDescendant.symbol.updateScope(finalScope)
	...
	return this.letter.gameMessage.playChannel() + this.nodeIdentifier()
    }
	...
	var node = this
	while (true) {
	    suffices.unshift (node.symbol.nonterm.id)
	    if (typeof(node.parent) == 'undefined')
		break;
	    suffices.unshift ('s' + node.siblingIndex)
	    suffices.unshift ('r' + node.sourceRule.ruleIndex)
	    node = node.parent
	}
	...
	var msgNode = null;
	if (this.symbol instanceof lw.NontermReference) {
	    msgNode = { name: this.symbol.nonterm.id,
		     type: "nonterm" }
	...
	if ('rule' in msgNode) {
		...
		if (this.expansionRule.ruleIndex != msgNode.rule) {
	    	...
	    	this.symbol.nonterm.rules[msgNode.rule].expand (this.span, this.placeholderSpan, this);
    ...
    lw.Node.prototype.addMoveListener = function() {
	var letter = this.letter
	function moveHandler(msg) { letter.root.expandFromTreeJSON(msg) }
	this.letter.addMoveListener (this.nodeIdentifier(), this.nodeChannel(), moveHandler)
    }

    lw.Node.prototype.cancelMoveListener = function() {
	if (this.isRemotelyControlled())
	    this.letter.cancelMoveListener (this.nodeIdentifier())
    }
	...
	var addNode = function(node,parent,x) {
	    var attr = {Type:"Choice",Player:node.symbol.random()?"Computer":"Human",Expanded:node.leaf?"not yet":"yes",stash:{node:node}}
	    var activeRules = 0
	...
	var addCode = function(node,x) {
	    ...
	    attr["Evaluates to"] = quoteHtml(isUndef ? "" : evalsTo.toString())
