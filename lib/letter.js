var LetterWriter = (function(){
	...
	nav.Message.prototype.populatePlayerList = function() {
	    ...
	    $("#pendingGrammarText").text('Waiting on channel #' + this.channel + ": got " + plural(j,"player") + ", need " + n + (n>0 ? "" : " (waiting for grammar)"))
	    $("#playerList").empty()
...
//			parentNode.options.push (new lw.Option (rule, option));
		...
		continueButton.innerText = "Continue" + ((letter.grammar.isSinglePlayer() || !letter.grammar.playingSolo()) ? "" : (" Player " + (1 + parentNode.role)))
