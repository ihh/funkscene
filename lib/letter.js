var LetterWriter = (function(){
    ...
    // editor methods
    lw.Grammar.prototype.newEditor = function(parentIdPrefix) {
	return new lw.GrammarEditor (this, parentIdPrefix);
    }
	...
	var isStart = nonterm.id == lw.defaultStart() || nonterm === grammar.start;
	...
	this.preambleDiv =
	    addControl ({property: "preamble",
			 inputDiv: $('<textarea rows="6"/>').addClass('main'),
	...
	this.placeholderDiv =
	    addControl ({property: "placeholder",
			 inputDiv: $('<textarea rows="1"/>').addClass('main'),
	...
	var addRuleLink = $('<a href="#">Add another option</a>')
	    .attr("title","Click to add another hint=>expansion.").tooltip()
	...
	var deleteNontermLink = $('<a href="#">Delete this phrase</a>')
	    ...
	    .click(function(e){
		e.preventDefault();
		var incoming = nonterm.incomingNonterms(grammar);
		if (incoming.length) {
		    ...
		    if (nonterm.isLooseEnd() ? false : !confirm ("Really, delete "+nonterm.asText()+" from the phrasebook?"))
			return;
	...
	nontermDiv
	    ...
	    .append($('<span class="addlink"></span>').append($('<i></i>').append(addRuleLink)))
	...
	function addProperty(editor,name,property,controlInfo,tooltip,filter) {
		...
		getInput = function() { return e.target.checked }
	    }
	    propDiv.input = inputDiv;
	    propDiv.info = addControl ({ property: property,
					 name: name,
					 isForm: true,
					 inputDiv: inputDiv,
					 tooltip: tooltip })
	...
	function addControl(h) {
	    ...
	    var info = extend ( { isForm: false,
				  ...
				  change: function(e){
				      var oldVal = info.get()
				      var newVal = filter(info.getInput())
		...
		.val(info.get())
		.change(info.change)
	    info.controlDiv = info.isForm ? $('<form action="#">').append(inputDiv) : inputDiv
	    info.parentDiv
		.append (info.containerDiv = $('<div/>').addClass("NontermControl").addClass(info.property+"PropertyEditor")
			 .attr("title",info.tooltip).tooltip()
			 .append(info.nameDiv = $('<div/>').addClass("NontermControlName")
				 .append('<i>'+info.name+'</i>'))
			 .append(info.valueDiv = $('<div/>').addClass("NontermControlValue")
				 .append(info.controlDiv))
			.append(info.warningDiv = $('<div/>').addClass("NontermControlWarning")))
	    return info
	}
	this.addControl = addControl

	addProperty (this, "Phrase @", "id", {type:"text"}, "The word used to identify this phrase.", grammarEditor.nontermRenamingFilter(nonterm))
...
*/
	...
	var hintInputDiv = $('<textarea rows="2"/>').attr({name: "hint" + rule.id})
	var rhsInputDiv = $('<textarea rows="2"/>').attr({name: "rhs" + rule.id})
	...
	function validateHint(random) {
	    ...
	    var hintWarningDiv = ruleEditor.hintInputWarningDiv
		...
		var weightFunc;
		try {
		    weightFunc = LetterWriter.parser.parse (rule.hint, "sum_expr");
		} catch (e) {
	...
	rhsInputDiv.autocomplete ({ change: rhsInputChange,
					...
					var match = /^(.*)(@[a-zA-Z0-9]*)$/.exec (request.term)
					var suggestions = []
					if (match) {
					    var typed = match[1]
					    var prefix = match[2]
	...
	function addRuleControl(info) {
	    ruleEditor.controlInfo[info.property] = nontermEditor.addControl (extend ({owner:rule}, info))
    ...
    lw.GrammarEditor = function(grammar,parentIdPrefix) {
...
//	    handle: "h3",
	...
	parentNode.date = new Date(); // ditto timestamp
    ...
    lw.Nonterm.prototype.isLooseEnd = function() {
	if (this.hasOwnProperty("preamble")
	    || this.hasOwnProperty("placeholder")
	    || this.prompt != this.defaultPrompt)
	    return false;
	for (var i = 0; i < this.rules.length; i++)
	    if (this.rules[i].hasOwnProperty("hint")
		|| this.rules[i].rhs.length > 0)
		return false;
	return true;
    }

    lw.Nonterm.prototype.incomingNonterms = function(grammar) {
	var incoming = [];
	for (var i = 0; i < grammar.nonterms.length; ++i) {
	    var nonterm = grammar.nonterms[i];
	    var found = false;
...
//	console.log ("Node collapsed: " + this.span.outerHTML);
    ...
    lw.Node.prototype.iteratePost = function(f) {
	for (var i = 0; i < this.child.length; ++i)
	    this.child[i].iteratePost (f);
	f.apply (this);
    }

    lw.Node.prototype.iteratePre = function(f) {
	f.apply (this);
	for (var i = 0; i < this.child.length; ++i)
	    this.child[i].iteratePre (f);
    }

    lw.Node.prototype.iterateChild = function(f) {
	for (var i = 0; i < this.child.length; ++i)
