var LetterWriter = (function(){
	...
	var preambleDiv = $('<textarea rows="6"/>').addClass('main')
	    ...
	    .attr("title","This is the preamble text, displayed wherever " + nid + " occurs in the narrative, before any choice is presented to the player.").tooltip()
	    .change(function(e){ nonterm.preamble = e.target.value });

	var placeholderDiv = $('<textarea rows="1"/>').addClass('main')
	    .val(nonterm.placeholder)
	    .attr("title","This is the placeholder text, displayed after the preamble and before the prompt. The placeholder is visible only until " + nid + " is expanded; it disappears when the player makes a choice. The preamble flows seamlessly into the placeholder; the player will not see any difference between the placeholder text and preceding text (at least, not until they make a choice).").tooltip()
