...
//	console.log ("Node collapsed: " + this.span.outerHTML);
	...
	var addNode = function(node,parent,x) {
	    addDebugTreeNode(makeNontermId(node),makeNontermId(parent),{label:node.symbol.nonterminal.asText(),x:x,color:node.leaf?"#00cc00":(node.symbol.random()?"#ff88ff":"#ff8888"),size:node.leaf?2:1}) }
	...
	var addCode = function(node,x) {
	    addDebugTreeNode(makeCodeId(node),makeNontermId(node.parent),{label:makeCodeLabel(node),x:x,color:(node.symbol instanceof LetterWriter.ParamAssignment)?"#0000ff":"#0088cc"}) }
