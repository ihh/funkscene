var LetterWriter = (function(){
	...
	function addProperty(editor,name,property,type) {
	    ...
	    var propNameDiv = propDiv.name = $('<div/>').addClass("propname").append(name);
	    var propValueDiv = propDiv.value = $('<div/>').addClass("propvalue");
	    var inputDiv = $('<input type="' + type + '"/>').attr("name",name);
	    if (type == "text") {
		inputDiv.attr("value",value);
		inputDiv.change(function(e){ nonterm[property] = e.target.value });
	    } else {
...
//	addProperty (this, "Phrase", "@" + nonterm.id);
	addProperty (this, "Placeholder", "placeholder", "text");
	...
	var hintInputDiv = $('<input type="text"/>').attr("name","hint" + rule.id).attr("value",rule.hint);
	var rhsInputDiv = $('<input type="text"/>').attr("name","rhs" + rule.id).attr("value",rhsAsText);
	var limitInputDiv = $('<input type="text"/>').attr("name","limit" + rule.id).attr("value",("maxUsage" in rule) && rule.maxUsage > 0 ? rule.maxUsage : "");

	// TODO: hookup change events
