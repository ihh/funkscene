...
//	    handle: "h3",
    ...
    lw.Rule.prototype.weight = function(node) {
	var weight = 1;
	if (letter.ruleActive (this)) {
		...
		weight = weightFunc.evaluate (node);
	...
	while (rhsQueue.length) {
		...
		symNode.leaf = false;
		var rule = sym.nonterminal.randomRule (symNode);
    ...
    lw.ParamFunc.prototype.evaluate = function(node) {
	switch (this.op) {
	case "*": return this.l.evaluate(node) * this.r.evaluate(node);
	case "/": return this.l.evaluate(node) / this.r.evaluate(node);
	case "+": return this.l.evaluate(node) + this.r.evaluate(node);
	case "-": return this.l.evaluate(node) - this.r.evaluate(node);
	case "!": return 1 - this.l.evaluate(node);
	case "$": return node.letter.paramValue[this.param];
    ...
    lw.Nonterm.prototype.randomRule = function(node) {
	var total = 0;
	var weight = [];
	for (var i = 0; i < this.rules.length; ++i) {
	    var w = this.rules[i].weight(node);
	    weight.push(w);
	    total += w;
	}
	...
	return "$" + this.id + " = " + this.value.asText() + "\n"
    }
