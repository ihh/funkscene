var LetterWriter = (function(){
	...
	if ('className' in elt)
	    elt.className = elt.className.replace(theClass,"")
    }
	...
	nav.Message.prototype.populatePlayerList = function() {
	    ...
	    $("#pendingGrammarText").text('Waiting on channel #' + this.channel + ": got " + plural(j,"player") + ", need " + n)
	    $("#playerList").empty()
...
//	setLocation()
	...
	for (var rhsPos = 0; rhsPos < this.rhs.length; ++rhsPos) {
	    ...
	    function symNodeIsVisible() { return typeof (lastPauseNode()) == "undefined" }
...
//		    console.log("Pushing delayedExpand of " + symNode.nodeIdentifier())
	    ...
	    // player- or remotely-controlled symbol (Term or NontermReference)
...
//		'top': node.displayY+15
		...
		for (var i = 0; i < highlightSpan.length; ++i)
		    addClassToElement(highlightSpan[i], "parseTreeHighlight")
	    }
	}
	...
	function hideNodeInfo() {
	    ...
	    popUp = false;
	    for (var i = 0; i < highlightSpan.length; ++i)
		removeClassFromElement (highlightSpan[i], "parseTreeHighlight")
	    highlightSpan = []
	}
