...
//	addProperty (this, "Phrase", "@" + nonterm.id);
	...
	rhsInputDiv.change(function(e){
		...
		for (var i = 0; i < newRhs.length; ++i) {
			...
			var nonterm = sym.nonterminal;
			if (nonterm.anonymous)
			    nonterm.id = grammar.createAnonId();
			if (!(nonterm.id in grammar.nonterm))
			    grammarEditor.addNonterm (sym, rule.lhs);
		    }
		}
	    }
...
//	    handle: "h3",
	...
	var grammar = this.grammar;
	var siblingDiv;
	nonterm.addRule("",[]);  // give it an empty rule or it won't be displayed
	extend (nonterm, nontermRef.props);  // migrate all the properties into the nonterminal
	nontermRef.props = {};
	nontermRef.buildAccessors();  // a bit hacky, but the compiler doesn't know about the editor, so
